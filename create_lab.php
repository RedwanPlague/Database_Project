<?php
    ...
    if(isset($_GET["name"]) && isset($_GET["email"]) && isset($_GET["phone"]) && isset($_GET["password"]) && isset($_GET["confirm_password"])) {
        ...
        $query = pg_query($db, "SELECT email FROM labs");
        $matched = false;
        ...
        if($matched == false) {
            ...
            $row = pg_fetch_row(pg_query($db, "SELECT lab_id FROM labs WHERE email = $email"));
            $id = $row[0];
...
<html lang="en">
    <head>
        <title> create lab account </title>
    </head>

    <body>
        <form name="form" action="create_lab.php" method="get" onsubmit="return validate()">
