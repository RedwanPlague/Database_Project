    ...
    if(isset($_GET["email"]) && isset($_GET["password"])) {
        ...
        if($db) {
            ...
            while($found != true && $row = pg_fetch_row($query)) {
                if(($selectOption == "patient" || $selectOption == "lab admin") && $email == $row[2] && (password_verify($password, $row[5]) || $row[5] == $password)) {
                    $id = $row[0];
                    $found = true;
                }

                if(($selectOption == "doctor") && $email == $row[2] && (password_verify($password, $row[4]) || $row[4] == $password)) {
                    $id = $row[0];
                    $found = true;
                }

                if(($selectOption == "collector") && $email == $row[3] && (password_verify($password, $row[5]) || $row[5] == $password)) {
                    $id = $row[0];
                    $found = true;
                }
            }
